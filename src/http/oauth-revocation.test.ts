...
async function connectAsClient(
  /**
   * Sending the RFC 8707 resource indicator is what decides the token shape:
   * with it the provider issues an audience-bound JWT (Codex, MCP Inspector),
   * without it an opaque random string (Claude Code). They are validated by
   * completely different code, so both need covering.
   */
  bindResource = true,
  ...
  const params = new URLSearchParams({
    ...
    state: "test",
    ...(bindResource ? { resource: `${server.url}/api/mcp` } : {}),
  });
...
describe("revoking a connector that holds a JWT", () => {
  ...
  it("ends an opaque token's access too", async () => {
    // The other half of the surface. This path always looked the token up in
    // the database, so revocation already reached it — worth pinning so a
    // future change cannot quietly make it stateless the way the JWT path was.
    const { clientId, accessToken } = await connectAsClient(false);
    expect(accessToken.split(".")).not.toHaveLength(3);

    expect((await toolsList(accessToken)).status).toBe(200);
    await rpc(server, "revokeOAuthClient", [clientId], cookie);
    expect((await toolsList(accessToken)).status).toBe(401);
  }, 60_000);
