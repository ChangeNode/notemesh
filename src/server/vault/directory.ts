...
import { isoInZone, modifiedFor } from "./modified";
...
export interface DirectoryEntry {
  ...
  size: number;
  /** Present only when a folder's time is its own mtime because nothing beneath it is indexed. */
  modifiedFrom?: "folder";
...
/** Newest time and total bytes the index holds under each immediate child of prefix. */
function beneathByChild(prefix: string): Map<string, Beneath> {
  const d = db();
  const rows = [
    ...(d
      .prepare("SELECT path, modified, size FROM notes WHERE substr(path, 1, ?) = ?")
      .all(prefix.length, prefix) as { path: string; modified: number | null; size: number }[]),
    ...(d
      .prepare("SELECT path, modified, size FROM attachments WHERE substr(path, 1, ?) = ?")
      .all(prefix.length, prefix) as { path: string; modified: number | null; size: number }[]),
  ];
  ...
  for (const row of rows) {
    ...
    const have = out.get(child) ?? { modified: null, size: 0 };
    have.size += row.size;
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      if (under?.modified != null) {
        out.push({ name: entry.name, path: rel, kind: "folder", modified: isoInZone(under.modified, timeZone), size: under.size });
      } else {
        ...
        out.push({
          ...
          path: rel,
          kind: "folder",
          modified: isoInZone(Math.round(st.mtimeMs), timeZone),
          size: under?.size ?? 0,
          modifiedFrom: "folder",
        });
      }
